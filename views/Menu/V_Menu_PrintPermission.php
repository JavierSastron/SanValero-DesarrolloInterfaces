...
function printPermission($menu, $data)
{
    ...
    foreach ($permissions as $permission) {
        if ($menu['id_Opcion'] == $permission['id_Opcion']) {
            ...
            if (isset($data[4])) {
                $checked = '';
                foreach ($data[4] as $userPermission) {
                    if ($userPermission['id_Permiso'] == $permission['id_Permiso']) {
                        $checked = 'checked';
                    }
                }
                $html .= '<input type="checkbox" id="c-userRole-'.$permission['id_Permiso'].'"class="ml-3"
                            onclick="changeUserPermission('.$permission['id_Permiso'].')"
                            '.$checked.'>';
                            
            }
            $html .='<span id="permissionText-' . $permission['id_Permiso'] . '"> -> ' . $permission['permiso'] . '</span>';
            if (isset($data[3])) {
                $checked = '';
                $roleRelated = '';
